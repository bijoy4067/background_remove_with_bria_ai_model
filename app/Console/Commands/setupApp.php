...
class setupApp extends Command
{
    ...
    public function handle()
    {
        ...
        if (!file_exists($venvPath)) {
            ...
            $pipCheck = trim(shell_exec('which pip 2>&1'));
            if (empty($pipCheck)) {
        ...
        $install = shell_exec("source {$activatePath} && pip install onnxruntime && pip install 'rembg=[cpu,cli]' 2>&1");
        $this->info($install);

        $this->info('Checking installations...');
        $packages = shell_exec("source {$activatePath} && pip list 2>&1");
        $this->info($packages);
